...
import { ui111ToBase64 } from '@utils';

const { cleanTimerSettingCode, setSleepPlanCode } = dpCodes;
interface Props {
  devInfo: DevInfo;
  preload?: boolean;
}
...
const composeLayout = (store: Store, component: React.ComponentType) => {
  ...
  class PanelComponent extends Component<Props> {
    ...
    async componentDidMount() {
      try {
        const res = await getDeviceCloudData('setSleepPlan');
        // 如果云端有数据，则不处理；如果云端没有数据，则设置默认值
        if (!res || !res.value) {
          // 设置默认值 默认时间为：22:00-06:00
          await TYSdk.device.putDeviceData({
            [setSleepPlanCode]: '0016000600',
          });
          await saveDeviceCloudData('setSleepPlan', { value: 'set' });
        }
      } catch (e) {}
    }
