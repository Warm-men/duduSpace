...
const { faultCode, starRollerCode, uploadRollerStateCode, rollerStateCode } = dpCodes;
const PopUp = (props: any) => {
  ...
  useEffect(() => {
    ...
    // Data[2]: 错误原因 0:正常 1:便仓未到位 2集便仓已满 3上盖异常 4猫进入 5滚筒无法到位 6猫靠近 7:计划时间冲突
  ...
  const getErrorBitmap2FaultList = (errorCode: number) => {
    const errorCodeList = [1, 2, 3, 5, 7, 8];
  ...
  const getButtons = () => {
    ...
    // Data[2]: 错误原因 0:正常 1:便仓未到位 2集便仓已满 3上盖异常 4猫进入 5滚筒无法到位 6猫靠近 7:计划时间冲突 8:反转
    const uploadRollerStateData = getUploadRollerState(uploadRollerState);
    ...
    const faultList = getErrorBitmap2FaultList(errorCode); // 1, 2, 3, 5, 7, 8
    ...
    if (operationMode.includes(rollerMode) && rollerState === 1) {
      ...
      if (isFault) {
        // case2: 其他异常，可复位、继续，异常code：1, 2, 3, 5, 7, 8
        return {
          button: Buttons.resetPopAndContinue,
          text: textFault,
          disabled: true,
