...
const Record: React.FC = () => {
  ...
  const getData = async () => {
    const dpIdArr = [cat_close_time, manual_feed_report, plan_feed_report].map(
      item => schema[item].id
    );
          ...
          if (+item.dpId === +dpIdArr[0]) {
            ...
            dataItem = {
              ...
              info: {
                ...
                num: eatInfo.parts,
                method: +item.dpId === +dpIdArr[1] ? 'manual' : 'plan',
                fault: getFaultStrings(fault, eatInfo.fault, false),
              },
            };
          }
